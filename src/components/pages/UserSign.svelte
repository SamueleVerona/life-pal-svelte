...
<section class="section-sign flex flex-col flex-1 items-center justify-center relative overflow-hidden bg-theme-bg" >
  <h2 class="section__logo text-transparent top-8 left-8 tracking-tight absolute text-[12rem] text-center leading-none break-spaces bg-logo-bg bg-clip-text ">Life<br />Pal</h2>
  <div class="ml-8 mb-8 absolute bottom-0 left-0 ">
    <svg class="translate-y-[2rem] opacity-50 p-6 w-[10rem] h-[10rem] rounded-full border-2 border-orange-500">
      <image class="w-full h-full"  href="src\assets\svelte.svg" />
    </svg>
    <svg class=" p-6 w-[10rem] h-[10rem] rounded-full border-2 border-cyan-500  opacity-50">
      <image class="w-full h-full"  href="src\assets\tailwind.svg" />
    </svg>
  </div>
  <section
  ...
  ">
    <legend class="fieldset-legend text-8xl pb-4 
   ...
   " required placeholder="user@email.com"  minlength="11"   bind:value={userEmail}/>
    ...
    bind:value={userPwd} />
    <label for="pwdtoggle" class="align middle mt-2 self-center">
      <span class="text-3xl text-zinc-500 font-semibold align-middle mr-2 " inert>Show/Hide</span>
      <input
      type="checkbox"
      name="pwdtoggle"
      on:click={()=> passwordVisible = !passwordVisible}
      class="checkbox w-9 h-9 bg-transparent text-transparent inline border-stone-600 rounded-full border-2 border-stone-600 checked:bg-stone-600 "
      />
    </label>
    <button class="
    btn
    ...
    rounded-xl" data-button-id="sign-submit"  on:mousedown={signAction}
    >
    {switchMode[0].toUpperCase() + switchMode .slice(1)}
    </button>
    <div class="toggle__container  mt-4 w-20 h-12 self-center " >
      <input type="checkbox" checked 
      class=" bg-white border-1 border-gray-300 rounded-full w-full h-full toggle toggle__switch before:rounded-3xl before:bg-emerald-300 checked:before:bg-[var(--theme-primary-dark)] " 
    ...
    >
      <div class="px-4 mb-2">
        <span class="badge rounded-2xl text-2xl h-10 w-block tracking-wide font-semibold" >Email: john.smith@email.com
        </span>
        <span class=" badge rounded-2xl text-2xl h-10 w-block font-semibold tracking-wide">Password: password</span>
      </div>
      <div class="px-4">
        <span class=" badge rounded-2xl text-2xl h-10 w-block  font-semibold tracking-wide">Email: jane.doe@email.com</span>
        <span class="  badge rounded-2xl text-2xl h-10 w-block  font-semibold tracking-wide">Password: passphrase</span>
      </div>
    </section>
  </section>
</section>
